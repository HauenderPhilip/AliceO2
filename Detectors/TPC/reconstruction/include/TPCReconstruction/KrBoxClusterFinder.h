...
class KrBoxClusterFinder
{
 ...
 public:
  ...
  /// Set Function for minimal charge required for maxCharge of a cluster
  void setMaxTimes(int maxTimes) { MaxTimes = maxTimes; }
  ...
  void fillADCValueInTempSlice(int cru, int rowInSector, int padInRow, float adcValue);
  void fillTempTimeSlice(std::vector<o2::tpc::Digit>& eventSector, const int timeSlice);
  void clearTempTimeSlice();
 ...
 private:
  ...
  std::array<std::array<float, MaxPads>, MaxRows> mTempTimeSlice;
  ...
  std::deque<std::array<std::array<float, MaxPads>, MaxRows>> mSetOfTimeSlices{};
