...
void KrBoxClusterFinder::fillTempTimeSlice(std::vector<o2::tpc::Digit>& eventSector, const int timeSlice) {
  ...
  for (std::vector<o2::tpc::Digit>::iterator digit = eventSector.begin(); digit != eventSector.end();) {
    ...
    // We don't need this digit anymore:
    digit = eventSector.erase(digit);
  }
  return;
}
...
std::vector<std::tuple<int, int, int>> KrBoxClusterFinder::findLocalMaxima(bool directFilling, const int timeOffset)
{
  ...
  // loop over whole mSetOfTimeSlices the find clusters
  // for (int iTime = 0; iTime < MaxTimes; iTime++) { //mSetOfTimeSlices.size()
  int iTime = mMaxClusterSizeTime;
